...
class PCheckins(Resource):
    """It contain all the api carryign the activity with aand specific patient"""

    def get(self):
        """Api to retive all the patient from the database"""

        patients = conn.execute("SELECT pat_id, pat_first_name, pat_last_name, pat_ph_no, pat_date, pat_dob, checkin FROM patient  ORDER BY pat_id").fetchall()
        return patients
...
class PCheckin(Resource):
    ...
    def put(self,id):
        ...
        patientInput = request.get_json(force=True)
        pat_checkin = patientInput['pat_checkin']
        conn.execute("UPDATE patient SET checkin=? WHERE pat_id=?",
                     (pat_checkin, id))
        conn.commit()
        return patientInput
